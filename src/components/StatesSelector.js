...
class StatesSelector extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      this.props.states.length === states.length &&
      isLoading === this.props.isLoading &&
      countries.length === this.props.countries.length &&
      countryId === this.props.countryId
    ) {
      this.props.getStateList(900, 1, nextProps.countryId);
    } else {
      if (states.length !== 0) {
                ...
                {states &&
                  states.map((item, index) => {
                    return (
                      <MenuItem
                        key={index}
                        value={`${item.objectId},${item.nameMmUni}`}
                      >
                        {isZawgyi() ? item.nameMmUni : item.nameMmZawgyi}
                      </MenuItem>
                    );
                  })}
              </Select>
